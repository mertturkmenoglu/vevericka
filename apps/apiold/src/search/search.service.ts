...
export class SearchService {
  ...
  async searchPosts(
    ...
    const postIds: string[] = searchResults.hits.hits.map((hit) => hit._source?.id).filter(Boolean) as unknown as string[];
  ...
  async searchUsers(
    ...
    const userIds = searchResults.hits.hits.map((hit) => hit._source?.id).filter(Boolean) as unknown as string[];
