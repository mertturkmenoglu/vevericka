<template>
  <v-card flat>
        ...
        <v-card-title>
          ...
          <v-dialog v-model="languageDialog" max-width="600px">
            <template v-slot:activator="{ on, attrs }">
              <v-btn text color="deep-orange" class="ml-2" v-bind="attrs" v-on="on"> + </v-btn>
            </template>
            <v-card>
              <v-card-title>
          ...
          <v-list-item v-for="(lang, idx) in user.languages" :key="idx">
            <v-list-item-content>
              <v-list-item-title>
                ...
                <v-icon color="deep-orange" right small @click="deleteLanguage(idx)">
                  mdi-close
                </v-icon>
              </v-list-item-title>
            </v-list-item-content>
        ...
        <v-card-title>
          ...
          <v-dialog v-model="wishToSpeakDialog" max-width="600px">
            ...
            <v-card>
              <v-card-title>
                <span class="headline">
                  {{ $t("settings.languages.wish_to_speak.dialog.title") }}
                </span>
              </v-card-title>
              <v-card-text>
                <v-container>
          ...
          <v-list-item v-for="(l, idx) in user.wishToSpeak" :key="idx">
            <v-list-item-content>
              <v-list-item-title>
                <v-icon color="deep-orange"> mdi-chevron-right </v-icon>
                {{ l }}
                <v-icon color="deep-orange" right @click="deleteWishToSpeak(idx)">
                  mdi-close
                </v-icon>
              </v-list-item-title>
            </v-list-item-content>
    ...
    <v-card-actions>
      <v-spacer />
      <v-btn outlined small color="deep-orange" class="mr-2" @click="update">
        {{ $t("settings.update") }}
      </v-btn>
    </v-card-actions>
  </v-card>
...
export default class SettingsLanguages extends Vue {
  ...
  readonly availableProficiencies = ['Elementary', 'Intermediate', 'Professional', 'Native'];
